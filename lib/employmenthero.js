...
// TODO: replace with the exact scopes shown in the owner's Developer Portal
// scope picker when the app is created — do not add scopes beyond the
// narrowest read-only set that covers roster/shift data.
const SCOPES_PLACEHOLDER = "TODO_CONFIRM_SCOPES_AT_APP_CREATION";

export function buildAuthorizeUrl(env, redirectUri, state, scopes) {
  const url = new URL(AUTHORIZE_URL);
  url.searchParams.set("response_type", "code");
  url.searchParams.set("client_id", env.EMPLOYMENT_HERO_CLIENT_ID);
  url.searchParams.set("redirect_uri", redirectUri);
  url.searchParams.set("scope", scopes || SCOPES_PLACEHOLDER);
