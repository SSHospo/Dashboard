...
import { localDateAndWeekday } from "./periods.js";
 ...
 */
/**
 * Same as fetchLineItemSalesByCategory, but bucketed by the local hour
 * (0-23, in the venue's own timezone) each order was placed in — powers the
 * hour-by-hour Sales x Labour view. Kept as its own function rather than
 * having fetchLineItemSalesByCategory call this and sum the buckets, so the
 * already-reconciled plain version is never put at risk by this addition.
 * Bucketed on the ORDER's created_at (Square doesn't timestamp individual
 * line items) — every line item in one order lands in the same hour bucket.
 * Returns Map<hour, Map<categoryName, cents>>.
 */
export async function fetchLineItemSalesByCategoryHourly(accessToken, locationIds, startISO, endISO, categoryNameByVariationId, timezone) {
  const centsByHourAndCategory = new Map();
  let cursor;
  do {
    const res = await fetch(`${PRODUCTION_HOST}/v2/orders/search`, {
      method: "POST",
    ...
    for (const order of body.orders || []) {
      ...
      const { hour } = localDateAndWeekday(createdMs, timezone);
      let byCategory = centsByHourAndCategory.get(hour);
      if (!byCategory) {
        byCategory = new Map();
        centsByHourAndCategory.set(hour, byCategory);
      }
      for (const li of order.line_items || []) {
        ...
        byCategory.set(categoryName, (byCategory.get(categoryName) || 0) + cents);
      }
    }
    cursor = body.cursor;
  } while (cursor);
  return centsByHourAndCategory;
}
